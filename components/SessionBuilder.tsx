...
export const SessionBuilder = ({ sessions, timers, onCreate, onRemove, onDuplicate }: Props) => {
  ...
  const totalDurationMs = segments.reduce(
    (total, segment) => total + segment.durationMs * segment.repetitions + (segment.restMs ?? 0),
    0
  );
    ...
    <div className="space-y-4 rounded-2xl border border-slate-200 bg-white/70 p-4 shadow-card dark:border-slate-800 dark:bg-slate-900/60">
      <div className="flex items-center justify-between gap-2">
        <div>
          ...
          <h3 className="text-lg font-bold text-slate-900 dark:text-slate-50">Composer vos sessions</h3>
        </div>
      ...
      <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
        <div className="space-y-3 rounded-xl border border-dashed border-primary-200 bg-primary-50/60 p-3 dark:border-primary-900/60 dark:bg-primary-500/5">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-xs font-semibold uppercase tracking-wide text-primary-700 dark:text-primary-200">Timers existants</p>
              <h4 className="text-base font-bold text-slate-900 dark:text-slate-50">Ajouter un compte à rebours</h4>
            </div>
          ...
          {availableCountdowns.length === 0 ? (
            <p className="text-sm text-slate-600 dark:text-slate-300">
              Créez d&apos;abord un compte à rebours dans l&apos;onglet Timers pour l&apos;enchaîner ici.
            </p>
          ) : (
          ...
          <p className="text-xs text-slate-500 dark:text-slate-400">Besoin d&apos;un nouveau timer ? Créez-le depuis l&apos;onglet Timers puis revenez lier vos segments.</p>
        </div>

        <div className="space-y-3 rounded-xl border border-dashed border-slate-200 p-3 dark:border-slate-800">
          <div>
            <p className="text-xs font-semibold uppercase tracking-wide text-slate-600 dark:text-slate-400">Segment ad hoc</p>
            <p className="text-sm text-slate-600 dark:text-slate-300">Utilisez ce formulaire rapide pour créer un segment dédié à cette session.</p>
          </div>
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) => setSegmentForm((prev) => ({ ...prev, minutes: parseInt(e.target.value, 10) || 0 }))}
                className="mt-1 w-full rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm text-slate-800 shadow-sm focus:border-primary-500 focus:outline-none dark:border-slate-700 dark:bg-slate-900 dark:text-slate-100"
              />
            </div>
            <div>
              ...
              <input
                ...
                onChange={(e) => setSegmentForm((prev) => ({ ...prev, seconds: parseInt(e.target.value, 10) || 0 }))}
                className="mt-1 w-full rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm text-slate-800 shadow-sm focus:border-primary-500 focus:outline-none dark:border-slate-700 dark:bg-slate-900 dark:text-slate-100"
              />
            </div>
            <div>
              ...
              <input
                ...
                onChange={(e) => setSegmentForm((prev) => ({ ...prev, repetitions: parseInt(e.target.value, 10) || 1 }))}
                className="mt-1 w-full rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm text-slate-800 shadow-sm focus:border-primary-500 focus:outline-none dark:border-slate-700 dark:bg-slate-900 dark:text-slate-100"
              />
            </div>
          </div>
          <div className="flex items-center gap-3">
            ...
            <span className="text-xs text-slate-500 dark:text-slate-400">Chaque segment inclura {delayBetween}s de repos.</span>
          </div>
        </div>
      </div>
      ...
      <div className="rounded-xl border border-slate-200 bg-white/70 p-3 shadow-inner dark:border-slate-800 dark:bg-slate-900/60">
        <div className="flex flex-wrap items-center justify-between gap-2">
          <div className="space-y-1">
            <p className="text-xs font-semibold uppercase tracking-wide text-slate-500 dark:text-slate-400">Timeline</p>
            <p className="text-sm text-slate-700 dark:text-slate-200">Durée totale : {formatDuration(totalDurationMs)}</p>
          </div>
        ...
        {segments.length > 0 && (
          <div className="mt-3 flex h-3 overflow-hidden rounded-full bg-slate-100 dark:bg-slate-800">
            {segments.map((segment) => {
              const segmentWidth = Math.max(10, (segment.durationMs / (totalDurationMs || 1)) * 100);
              return (
                <span
                  key={segment.id}
                  className="h-full"
                  style={{ width: `${segmentWidth}%`, background: segment.color ?? '#22c55e' }}
                  title={`${segment.label} · ${formatDuration(segment.durationMs)} (${segment.repetitions}x)`}
                />
              );
            })}
          </div>
      ...
      <div className="flex items-center justify-between">
        ...
        <button
          ...
          title={segments.length === 0 ? 'Ajoutez au moins un segment' : ''}
          className="rounded-full bg-gradient-to-r from-primary-500 to-primary-700 px-4 py-2 text-sm font-semibold text-white shadow-card transition hover:-translate-y-0.5 hover:shadow-lg disabled:cursor-not-allowed disabled:opacity-60"
        >
          Créer la session
        </button>
      </div>
