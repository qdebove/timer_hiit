...
export const TimerForm = ({ onCreate }: Props) => {
  ...
  return (
    <Card variant="surface" className="p-4 sm:p-5">
      <form onSubmit={handleSubmit} className="space-y-4">
                ...
                {[
                  ...
                  return (
                    <button
                      ...
                      className={`flex w-full items-center gap-2 rounded-xl border px-3 py-2 text-left text-xs sm:text-sm font-semibold leading-snug whitespace-normal transition ${
                        active
        ...
        <div className="space-y-3 rounded-2xl border border-slate-100 bg-slate-50/70 px-4 py-3 dark:border-slate-800 dark:bg-slate-900/50">
          ...
          <div className="flex flex-col items-start gap-3 border-t border-slate-200 pt-3 text-xs text-slate-500 dark:border-slate-700 dark:text-slate-400 sm:flex-row sm:items-center sm:justify-between">
            <p>
              {kind === 'stopwatch'
                ? 'Chronomètre sans limite : ajoutez des étapes pendant la course.'
                : 'Le compte à rebours se base sur la durée indiquée.'}
            </p>
            <PillButton type="submit">
              {kind === 'stopwatch'
                ? 'Créer le chronomètre'
                : `Créer le timer (${formatDuration(computedDuration)})`}
            </PillButton>
          </div>
        </div>
      </form>
